...
class CloudWatchECS:
    """
    Encapsulates Amazon CloudWatch ECS functions.
    """
    def __init__(self, ecs, cloudwatchclient, default_values):
        metric_needed = {}
        ...
        for metric_name in metric_needed:
            ciname = ecs["clusterName"]
            cloudid = ecs["clusterArn"]

            if "DynamicCore" in metric_needed[metric_name]["MetricSpecifications"]:
                getattr(CloudWatchECS, metric_needed[metric_name]["MetricSpecifications"]["DynamicCore"])(
                    ecs, ciname, cloudid, default_values)
            else:
